...
### Cli
The [cli image](cli/Dockerfile) is intended to be used with cli tasks like running scripts or other tasks. Additionally it comes with some common binaries installed for database and other cli stuff.

### Web
The [web image](web/Dockerfile) runs an apache web server with all needed extensions enabled. Additionally it comes with SSL support (self signed) and some common binaries installed for database and other cli stuff. In the web image is also `localtunnel` installed to quickly start a local tunnel within the web server:
`lt --port 80`
